...
router.post('/ocr-jobs', auth.requireAuth, async function (req, res, next) {
  try {
    var body = req.body || {};
    ...
    // 案件权限和幂等查询并行执行，缩短创建任务的同步等待时间。
    var accessPromise = caseAccess.getCaseForUser(body.caseId, req.openid);
    var idempotencyKey = String(body.idempotencyKey || '');
    var existingPromise = idempotencyKey
      ? db.collection('ocr_jobs').where({
        caseId: body.caseId, requestedBy: req.openid, idempotencyKey: idempotencyKey,
      }).limit(1).get()
      : Promise.resolve({ data: [] });
    var accessResult = await Promise.all([accessPromise, existingPromise]);
    var existing = accessResult[1];
    if (existing.data && existing.data[0]) {
      return res.json({ code: 0, data: { jobId: existing.data[0]._id, status: existing.data[0].status, duplicate: true }, message: 'ok' });
    }
    // 这里只创建持久化任务，不在同步请求里读取历史证据哈希。
    // 历史去重信息由 OCR Worker 在后台加载，避免 callContainer 等待额外数据库查询。
    ...
    req.app.locals.ocrWorker.kick().catch(function (error) {
      console.error(JSON.stringify({
        event: 'ocr_worker_kick_failed',
        jobId: jobId,
        errorCode: error.code || 'OCR_WORKER_KICK_FAILED',
