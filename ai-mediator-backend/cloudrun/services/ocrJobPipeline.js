...
async function run(jobId, options) {
  ...
  metrics.observe('ocr_job_download_ms', Date.now() - downloadStartedAt);
  var exactSeen = {};
  var perceptualSeen = [];
  ...
  // 历史哈希在后台加载，不占用创建任务的同步请求窗口。
  var previousBatches = await db.collection('evidence_batches').where({ caseId: job.caseId }).get();
  (previousBatches.data || []).forEach(function (batch) {
    (batch.sourceHashes || []).forEach(function (hash) {
      if (hash) exactSeen[hash] = 'previous_batch';
    });
    (batch.perceptualHashes || []).forEach(function (hash) {
      if (hash) perceptualSeen.push({ hash: hash, index: 'previous_batch' });
    });
  });
  var detailed = [];
  var ocrStartedAt = Date.now();
  ...
  metrics.observe('ocr_job_vision_ms', Date.now() - ocrStartedAt);
