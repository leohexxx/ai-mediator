...
function createCollection(name) {
  if (config.localMode) {
    readCollection(name);
    return Promise.resolve({ created: false, local: true, name: name });
  }
  if (!db) return Promise.reject(initError || new Error('CloudBase database is unavailable'));
  if (typeof db.createCollection !== 'function') {
    return Promise.reject(new Error('CloudBase SDK does not support collection creation'));
  }
  return Promise.resolve(db.createCollection(name)).then(function (result) {
    return { created: true, local: false, name: name, result: result };
  });
}
