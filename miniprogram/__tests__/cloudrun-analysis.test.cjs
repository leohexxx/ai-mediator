...
async function run() {
  ...
  assert.ok(evidenceSource.includes("'/api/upload/ocr-jobs'"), '图片 OCR 应创建异步识别任务');
  assert.ok(evidenceSource.includes("'/api/upload/ocr-jobs/' + jobId"), '图片 OCR 应轮询异步识别结果');
  assert.ok(evidenceSource.includes('102002'), '图片 OCR 遇到 CloudRun 102002 应有幂等重试路径');
  assert.ok(evidenceSource.includes('重试必须复用幂等键'), 'OCR 重试必须复用幂等键避免重复任务');
  assert.ok(evidenceSource.includes("cloudRun.idempotencyKey('ocr_' + caseId)"), 'OCR 任务应在一次批次内固定幂等键');
  assert.ok(evidenceSource.includes('error.fileIds = fileIds'), '任务创建失败时应保留已上传 fileIds');
  assert.ok(uploadPageSource.includes('pendingOcrJobId'), '上传页必须保存待完成任务以支持离开页面后恢复');
  assert.ok(uploadPageSource.includes('ocrCreatePending'), '任务创建失败时上传页应保留可恢复状态');
  assert.ok(uploadPageSource.includes('onRetryCreateOcrJob'), '上传页应提供继续创建 OCR 任务入口');
  assert.ok(uploadPageSource.includes('OCR_COLLECTION_NOT_READY'), '集合未初始化时应显示明确提示');
